...
Дата проверки: 24 сентября 2026. Рабочая ветка: `feature/licensing-and-trial`.
...
- Активация, проверка и деактивация текущего Mac подключены к публичному Lemon
  ...
  повторно проверяет лицензию раз в 7 дней даже без перезапуска. После временной
...
На рабочем дереве 24 сентября полный `PasteQueueTests` прошёл: **97 тестов,
0 ошибок**. Команда:
...
- Тестовые Store ID, Product ID, Variant ID и checkout Lemon Squeezy проверены
  реальной тестовой активацией с лимитом 3 устройства. Production магазин,
  live IDs/checkout, доставка ключа, возвраты, политика приватности, канал
...
- `.github/workflows/ci.yml` проверяет PR и push в `main` на Apple Silicon:
  ...
  публикует артефакты; первый результат GitHub Actions нужно подтвердить после
  push ветки.
...
2. **Частично выполнено:** `project.yml` обновлён, проект сгенерирован,
   вычисленные Release-настройки сверены, Sparkle и production feed подключены.
...
5. Live IDs и checkout Lemon Squeezy заполнены в `project.yml`; пройти live-покупку,
